...
router.post("/login", (request, response) => {
  ...
  const query = `SELECT profile_image_url,joined_date,bio,username,uid,email,password FROM users WHERE username='${userEnteredUsername}'`;

  db.query(query, (err, res) => {
    if (err) return response.status(500).json({ message: "Something went wrong." });

    const { profile_image_url, uid, bio, email, username, password, joined_date } = res.rows.length > 0 ? res.rows[0] : [];
    ...
    bcrypt.compare(userEnteredPassword, password, (_, matched) => {
      ...
      return response.status(200).json({ profile_image_url, uid, username, bio, email, token, joined_date });
    });
  });
});
